...
import (
	...
	rexrayc "k8s.io/kubernetes/pkg/cloudprovider/providers/rexray"
	...
	"k8s.io/kubernetes/pkg/volume/rexray"
...
func ProbeRecyclableVolumePlugins(config componentconfig.VolumeConfiguration) []volume.VolumePlugin {
	...
	allPlugins = append(allPlugins, rexray.ProbeVolumePlugins()...)
...
func NewVolumeProvisioner(cloud cloudprovider.Interface, config componentconfig.VolumeConfiguration) (volume.ProvisionableVolumePlugin, error) {
	...
	case cloud != nil && rexrayc.ProviderName == cloud.ProviderName():
		return getProvisionablePluginFromVolumePlugins(rexray.ProbeVolumePlugins())
	}
	return nil, nil
}
